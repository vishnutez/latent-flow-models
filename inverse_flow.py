...
# Code to solve inverse problems using latent flow models (LFMs) as in latent diffusion models (LDMs)
...
# Function to sample using conditional measurment guidance
def sample_from_model_measurement_guidance(model, 
                                           x_0, 
                                           y, 
                                           operator, 
                                           model_kwargs, args):
    ...
    # Creating an approximate conditioanl velocity field using the measurements y, and the operator

    from conditional_sampler import ConditionalVF

    cvf = ConditionalVF(denoiser, y, operator).to(x_0.device)
...
if __name__ == "__main__":
    ...
    # DPS and inverse problem configs
    ...
    # model_config=args.dps_path+args.model_config
    # diffusion_config=args.dps_path+args.diffusion_config


    task_config = args.dps_path + args.task_config
    ...
    task_config['data']['root'] = args.dps_path + 'data/samples/'

    img = plt.imread(task_config['data']['root'] + args.file_id)
    ...
    # Convert y_n to image and save at data/observations/ directory

    y_n_image = y_n[0].detach().cpu().numpy()

    y_n_image = y_n_image - y_n_image.min()

    y_n_image = y_n_image / y_n_image.max()

    y_n_image = torch.FloatTensor(y_n_image)

    y_n_image = torch.unsqueeze(y_n_image, dim=0).permute(0,3,1,2)

    y_n_image = y_n_image[:,:3,:,:].cuda()

    # Save the noisy image

    torchvision.utils.save_image(y_n_image, "data/observations/{}.jpg".format(args.file_id), padding=0, nrow=8)
